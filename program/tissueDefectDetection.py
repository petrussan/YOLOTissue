...
if __name__ == '__main__':
	# Load class names.
	classesFile = "obj.tissuenames"
	classes = None
	with open(classesFile, 'rt') as f:
		classes = f.read().rstrip('\n').split('\n')

	# Load image.
	# cap = cv2.VideoCapture('openCV/tisReject.avi')
	cap=cv2.VideoCapture(1)

	# Give the weight files to the model and load the network using them.
	#modelWeights = "yolov4_1_3_416_416_static.onnx"
 	#net = cv2.dnn.readNet(modelWeights)
	net = cv2.dnn.readNet(model='yolov4-tiny-trainTissue_final.weights', config='yolov4-tiny-testTissue.cfg')
